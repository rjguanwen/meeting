...
3. **账号管理**：为每个部门 / 小组创建负责人账号（部门、小组都可建，一个组织只能有一个）
4. **会议管理 → 创建会议**：填写会议信息，在详情页「设置参会组织」
...
| `users` | 用户账号（`admin` / `leader`，leader 绑定组织） |
| `meetings` | 会议（draft / ongoing / finished / archived） |
| `meeting_orgs` | 会议参会组织 |
